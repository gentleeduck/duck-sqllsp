...
pub fn build() -> HashMap<&'static str, Entry> {
  ...
  macro_rules! t {
    ($label:expr, $doc:expr, $example:expr, $url:expr) => {
      ...
      if let Some(prev) = m.insert(
        ...
        panic!(
          "duplicate dsl-knowledge type entry {:?}: new doc {:?} would silently replace existing doc {:?}",
          $label, $doc, prev.doc
        );
      }
    };
  ...
  t!(
    "ANY",
    "Polymorphic pseudo-type accepted by functions; bound to a concrete type at call time.",
    "x ANY",
    pg("extend-type-system.html#EXTEND-TYPES-POLYMORPHIC")
  );
  t!(
    "ANYARRAY",
    "Polymorphic pseudo-type accepted by functions; bound to a concrete type at call time.",
  ...
  t!(
    ...
    "Polymorphic pseudo-type accepted by functions; bound to a concrete type at call time.",
    "x ANYCOMPATIBLEMULTIRANGE",
    pg("extend-type-system.html#EXTEND-TYPES-POLYMORPHIC")
  );
  t!(
    "ANYCOMPATIBLENONARRAY",
    "Polymorphic pseudo-type accepted by functions; bound to a concrete type at call time.",
    "x ANYCOMPATIBLENONARRAY",
    pg("extend-type-system.html#EXTEND-TYPES-POLYMORPHIC")
  );
  t!(
    "ANYCOMPATIBLERANGE",
    "Polymorphic pseudo-type accepted by functions; bound to a concrete type at call time.",
    "x ANYCOMPATIBLERANGE",
    pg("extend-type-system.html#EXTEND-TYPES-POLYMORPHIC")
  );
  t!(
    "ANYELEMENT",
  ...
  t!(
    "CSTRING",
    "Pseudo-type internal to PG -- not usable in user table columns.",
    "x CSTRING",
    pg("datatype-pseudo.html")
  );
  ...
  t!(
    "FDW_HANDLER",
    "Pseudo-type for the entry point of a feature handler. Cannot appear in user SQL.",
    "x FDW_HANDLER",
    pg("extend-type-system.html")
  );
  t!(
    "INDEX_AM_HANDLER",
    "Pseudo-type for the entry point of a feature handler. Cannot appear in user SQL.",
  ...
  t!(
    "JSONPATH",
    "JSONPath expression value -- used by jsonb_path_* functions.",
    "p JSONPATH",
    pg("datatype-json.html#DATATYPE-JSONPATH")
  );
  t!(
    "LANGUAGE_HANDLER",
    "Pseudo-type for the entry point of a feature handler. Cannot appear in user SQL.",
  ...
  t!(
    "NAME",
    "63-byte identifier -- internal PG type used in pg_catalog rows.",
    "-- pg_catalog only",
    pg("datatype-character.html")
  );
  t!(
    "NATIONAL CHAR",
    "SQL standard alias for CHARACTER (NCHAR = national char). PG treats them as plain text.",
    "c NATIONAL CHAR(20)",
    pg("datatype-character.html")
  );
  t!(
  ...
  t!(
    "PG_SNAPSHOT",
    "Visibility snapshot -- replaces deprecated txid_snapshot.",
    "snap PG_SNAPSHOT",
    pg("datatype-pg-lsn.html")
  );
  t!(
  ...
  t!(
    "REGCOLLATION",
    "Object-identifier alias type -- accepts a name + resolves to OID. Useful in queries against pg_catalog.",
    "t REGCOLLATION",
    pg("datatype-oid.html")
  );
  t!(
  ...
  t!(
    ...
    "SQL standard spelling for the time/timestamp variants (with/without time zone).",
  ...
  t!(
    "XID",
    "4-byte transaction id. Wraps around -- compare with `txid_*` fns or convert to `xid8` (PG13+) for full ordering.",
    "id XID",
    pg("datatype-oid.html")
  );
  t!(
    "XID8",
    "8-byte transaction id (PG13+) -- no wraparound concerns. Pair with `pg_snapshot`.",
    "id XID8",
    pg("datatype-oid.html")
  );
  t!(
    "ACLITEM",
    "Single ACL entry stored inside an aclitem[] (used by pg_class.relacl etc). Read via `aclexplode()` / `pg_get_userbyid()`.",
    "x ACLITEM",
    pg("ddl-priv.html")
  );
  ...
  t!(
    "HSTORE",
    "Key/value text map (hstore extension). Use jsonb instead in new code.",
    "tags HSTORE",
    "https://www.postgresql.org/docs/current/hstore.html"
  );
  t!(
    "VECTOR",
    "Dense float vector (pgvector extension). Specify dimensions: VECTOR(1536).",
    "embedding VECTOR(1536)",
    "https://github.com/pgvector/pgvector"
  );
  t!(
    "HALFVEC",
    "Half-precision (float2) vector (pgvector 0.7+). Half the storage of VECTOR.",
    "embedding HALFVEC(1536)",
    "https://github.com/pgvector/pgvector"
  );
  t!(
    "BIT_VEC",
    "Binary vector (pgvector 0.7+). Bit-packed for hamming/jaccard distance.",
    "fp BIT_VEC(64)",
    "https://github.com/pgvector/pgvector"
  );
  t!(
    "SPARSEVEC",
    "Sparse vector (pgvector 0.7+). Only stores nonzero indices.",
    "v SPARSEVEC(10000)",
    "https://github.com/pgvector/pgvector"
  );
  t!(
    "LTREE",
    "Hierarchical label tree (ltree extension). Use ~ and @> for ancestor queries.",
    "path LTREE",
  ...
  t!(
    "CITEXT",
    "Case-insensitive text (citext extension). UNIQUE/= compare folded.",
    "email CITEXT",
  ...
  t!(
    "CUBE",
    "Multi-dim cube (cube extension). Used by earthdistance.",
    "c CUBE",
    "https://www.postgresql.org/docs/current/cube.html"
  );
  t!(
    "EARTH",
    "earth-distance type (earthdistance extension).",
    "e EARTH",
  ...
  t!(
    "GEOMETRY",
    "PostGIS geometry. Specify subtype/SRID: GEOMETRY(Point, 4326).",
    "geom GEOMETRY(Point, 4326)",
    "https://postgis.net/docs/geometry.html"
  );
  t!(
    "GEOGRAPHY",
    "PostGIS geography (spherical, computations in meters).",
  ...
  t!(
    "HYPERTABLE",
    "TimescaleDB virtual table -- not a real column type; created via create_hypertable().",
    "-- see create_hypertable()",
