    ...
    for src in FIXTURES {
        ...
        for scope in resolve(&p.statements) {
            for b in scope.tables() {
                assert!(
                    !b.table.name.is_empty(),
                    "binding with empty table.name in `{src}`"
                );
            }
        }
    }
        ...
        for scope in resolve(&p.statements) {
            ...
            for n in &names {
                ...
                if let Some(b2) = scope.get(tname) {
                    assert_eq!(
                        b.table.name, b2.table.name,
                        "alias `{n}` and name `{tname}` disagree in `{src}`"
                    );
                }
            }
        }
...
fn property_select_from_table_yields_nonempty_scope() {
    for src in FIXTURES.iter().filter(|s| s.starts_with("SELECT") || s.starts_with("WITH")) {
        ...
        for scope in &scopes {
            assert!(
                !scope.is_empty(),
                "scope empty for `{src}` (parsed={:?})", scopes.len()
            );
        }
    }
}
