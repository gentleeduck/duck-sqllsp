...
fn build_chain(text: &str, rope: &Rope, offset: usize) -> SelectionRange {
  ...
  if let Some(r) = identifier_range(bytes, offset) {
    layers.push(r);
  }
  if let Some(r) = string_contents(bytes, offset) {
    push_dedup(&mut layers, r);
  }
  if let Some(r) = string_with_quotes(bytes, offset) {
    push_dedup(&mut layers, r);
  }
  if let Some(r) = call_arg_range(bytes, offset) {
    push_dedup(&mut layers, r);
  }
  if let Some(r) = paren_group(bytes, offset) {
    push_dedup(&mut layers, r);
  }
  if let Some(r) = clause_range(bytes, offset) {
    push_dedup(&mut layers, r);
  }
  if let Some(r) = statement_range(bytes, offset) {
    push_dedup(&mut layers, r);
  }
  ...
  for (s, e) in layers.into_iter().rev() {
    let r = Range { start: byte_to_position(rope, s), end: byte_to_position(rope, e) };
...
fn identifier_range(bytes: &[u8], offset: usize) -> Option<(usize, usize)> {
  let mut s = offset;
  while s > 0 && is_word(bytes[s - 1] as char) {
    s -= 1;
  }
  let mut e = offset;
...
fn call_arg_range(bytes: &[u8], offset: usize) -> Option<(usize, usize)> {
  ...
  if offset < inside_start || offset > inside_end {
    return None;
  }
  let mut depth = 0i32;
