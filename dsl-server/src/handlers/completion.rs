    ...
    if bytes[i] == b'$' && i + 1 < n {
      if bytes[i + 1] == b'{' {
        ...
        if j < n {
          ...
          let payload_start = body.as_bytes().iter().position(|&b| !b.is_ascii_digit()).unwrap_or(body.len());
...
mod tests {
  ...
  fn renders_default_text_placeholder() {
    assert_eq!(render_snippet_preview("SELECT ${1:col} FROM ${2:tbl};"), "SELECT col FROM tbl;");
  }
