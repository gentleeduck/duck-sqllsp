...
use tower_lsp::lsp_types::{
    DocumentOnTypeFormattingParams, Position, Range, TextEdit,
};
...
pub fn run(state: &ServerState, params: DocumentOnTypeFormattingParams) -> Option<Vec<TextEdit>> {
    ...
    if new_indent.is_empty() { return None; }
...
fn indent_unit(opts: &tower_lsp::lsp_types::FormattingOptions) -> String {
    if opts.insert_spaces {
        " ".repeat(opts.tab_size.max(1) as usize)
    } else {
        "\t".to_string()
    }
}
