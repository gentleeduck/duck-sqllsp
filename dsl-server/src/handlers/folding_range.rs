...
fn push_paren_blocks(src: &str, rope: &Rope, out: &mut Vec<FoldingRange>) {
    ...
    while i < n {
        ...
        if c == '\'' { i = skip_single_quoted(bytes, i); continue; }
        if c == '"' { i = skip_double_quoted(bytes, i); continue; }
        ...
        if c == '$' {
            if let Some(end) = skip_dollar_quoted(bytes, i) { i = end; continue; }
        }
        if c == '(' { stack.push(i); }
        else if c == ')' {
...
fn push_begin_end_blocks(src: &str, rope: &Rope, out: &mut Vec<FoldingRange>) {
    ...
    while i < n {
        ...
        let mut j = i;
        while j < n && is_word_cont(bytes[j] as char) { j += 1; }
        let word = &upper[i..j];
        match word {
            "BEGIN" => stack.push(i),
            "END" => {
...
fn skip_block_comment(bytes: &[u8], i: usize) -> usize {
    let n = bytes.len();
    let mut j = i + 2;
    while j + 1 < n && !(bytes[j] == b'*' && bytes[j + 1] == b'/') { j += 1; }
    (j + 2).min(n)
}
