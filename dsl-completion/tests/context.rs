use dsl_completion::context::{Context, detect};
use text_size::TextSize;

fn at_end(s: &str) -> TextSize {
...
fn predicate_after_where() {
  assert_eq!(detect("SELECT * FROM users WHERE ", at_end("SELECT * FROM users WHERE ")), Context::Predicate);
}

#[test]
fn predicate_after_on() {
  assert_eq!(detect("FROM a JOIN b ON ", at_end("FROM a JOIN b ON ")), Context::Predicate);
}
