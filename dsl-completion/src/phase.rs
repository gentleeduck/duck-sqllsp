...
fn tokenise(src: &str, start: usize, end: usize) -> Vec<Tok<'_>> {
    ...
    while i < end {
        ...
        if c == '/' && i + 1 < end && bytes[i + 1] == b'*' {
            i += 2;
            while i + 1 < end && !(bytes[i] == b'*' && bytes[i + 1] == b'/') { i += 1; }
            i = (i + 2).min(end);
            continue;
        }
        if c == '\'' {
            ...
            while i < end {
                let b = bytes[i];
                if b == b'\'' && (i == 0 || bytes[i - 1] != b'\\') { i += 1; break; }
                i += 1;
            }
        ...
        if c.is_ascii_digit() {
            let s = i;
            while i < end && (bytes[i].is_ascii_digit() || bytes[i] == b'.') { i += 1; }
            let _ = s;
        ...
        if is_word_start(c) {
            ...
            while i < end {
                ...
                if !bytes[i].is_ascii() { break; }
                if !is_word_cont(bytes[i] as char) { break; }
                i += 1;
            }
...
fn is_word_start(c: char) -> bool { c.is_alphabetic() || c == '_' }
fn is_word_cont(c: char) -> bool { c.is_alphanumeric() || c == '_' }
...
fn after_double_colon(src: &str, pos: usize) -> bool {
    ...
    while i > 0 && is_word_cont(bytes[i - 1] as char) { i -= 1; }
...
fn statement_start(src: &str, pos: usize) -> usize {
    ...
    while i < pos.min(bytes.len()) {
        let c = bytes[i] as char;
        if !in_double && c == '\'' && (i == 0 || bytes[i - 1] != b'\\') { in_single = !in_single; }
        else if !in_single && c == '"' { in_double = !in_double; }
        else if !in_single && !in_double && c == ';' { last_semi = i + 1; }
        i += 1;
    }
    last_semi
}
...
fn upper(s: &str) -> String { s.to_ascii_uppercase() }
