...
pub fn scope_from_text(src: &str) -> Option<Scope> {
    ...
    for (table, alias) in iter_table_bindings(src) {
        ...
        scope.bindings.entry(table).or_insert(Binding {
            alias: table_ref.name.clone(),
            table: table_ref,
        });
    }
...
fn match_keyword(chars: &[char], pos: usize, keywords: &[&str]) -> Option<usize> {
    ...
    for &kw in keywords {
        if matches_ci(chars, pos, kw)
            && chars.get(pos + kw.len()).map_or(true, |c| !is_word(*c))
        {
            return Some(kw.len());
        }
    }
...
fn read_table_with_alias(chars: &[char], pos: usize) -> Option<(String, String, usize)> {
    ...
    if matches_ci(chars, i, "AS") && chars.get(i + 2).map_or(true, |c| !is_word(*c)) {
        ...
        const STOPWORDS: &[&str] = &[
            ...
            "CROSS", "LATERAL", "USING", "GROUP", "ORDER", "LIMIT", "OFFSET",
            "HAVING", "UNION", "INTERSECT", "EXCEPT", "AS",
        ];
