...
fn find_enclosing_body(source: &str, pos: usize) -> Option<Body> {
    ...
    while i < bytes.len() {
        ...
        if c == '\'' {
            i += 1;
            while i < bytes.len() && !(bytes[i] == b'\'' && bytes[i - 1] != b'\\') { i += 1; }
            i = (i + 1).min(bytes.len());
            continue;
        }
        if c == '$' {
            ...
            if j < bytes.len() && bytes[j] as char == '(' {
                ...
                while k < bytes.len() && depth > 0 {
                    match bytes[k] as char {
                        ...
                        '\'' => {
                            k += 1;
                            while k < bytes.len() && !(bytes[k] == b'\'' && bytes[k - 1] != b'\\') { k += 1; }
                        }
                        _ => {}
                    }
                    k += 1;
...
fn current_entry<'a>(source: &'a str, body_start: usize, cursor: usize) -> Entry<'a> {
    ...
    while i < end {
        ...
        else if c == ',' && depth == 0 { last_split = i + 1; }
        i += 1;
    }
...
fn strip_trailing_word(s: &str) -> &str {
    ...
    while end > 0 && bytes[end - 1].is_ascii() && is_word(bytes[end - 1] as char) { end -= 1; }
...
fn is_complete_type_token(tok: &str) -> bool {
    ...
    if tok.ends_with(')') { return true; }
    matches!(upper.as_str(),
        "UUID" | "TEXT" | "DATE" | "TIME" | "TIMESTAMP" | "TIMESTAMPTZ" |
        "BOOLEAN" | "BOOL" | "INT" | "INTEGER" | "BIGINT" | "SMALLINT" |
        "SERIAL" | "BIGSERIAL" | "SMALLSERIAL" | "REAL" | "JSON" | "JSONB" |
        "BYTEA" | "INET" | "CIDR" | "MACADDR" | "INTERVAL" | "MONEY"
    )
}
