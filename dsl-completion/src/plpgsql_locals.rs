...
pub fn extract(source: &str, offset: usize) -> Locals {
  ...
  for needle in ["CREATE OR REPLACE FUNCTION ", "CREATE OR REPLACE PROCEDURE ", "CREATE FUNCTION ", "CREATE PROCEDURE "]
  {
    let mut from = 0usize;
...
fn parse_param_list(s: &str) -> Vec<(String, String)> {
  ...
  for part in split_top_level(s, ',') {
    ...
    // the name + type.
    let head = trimmed.split_once(" DEFAULT ").map(|(h, _)| h).unwrap_or(trimmed);
