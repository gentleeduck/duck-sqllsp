# Phase B batch 1: extract MERGE completion cluster Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: superpowers:executing-plans (inline, no subagents).

**Goal:** Pure code motion -- move the 9-function, self-contained MERGE completion cluster (engine.rs lines 8981-9239) into its own `dsl-completion/src/merge.rs` module, matching the existing `create_index.rs`/`create_table.rs`/`sources.rs` sibling-module convention (`pub mod X;` in `lib.rs`, `use crate::X;` + `X::fn_name(...)` call sites in `engine.rs`). **Zero logic changes** -- this is Stage 1 of Phase B (file-split only); Stage 2 (replacing the if-chain dispatch with an explicit registry) is deliberately separate future work, not part of this batch. This refines the spec's original single-stage sketch into two safer stages, given the real stakes (a 2073-test-covered live subsystem, not independent additive files).

**Spec:** `dsl-completion/docs/specs/2026-08-18-completion-engine-redesign-design.md` (Phase B).

**Verified before writing this plan:**
- `merge_target_and_source`, `merge_aliases`, `extract_first_ident`, `nth_ident`, `merge_update_set_lhs_slot`, `merge_update_set_rhs_expr_slot`, `merge_insert_col_list_slot`, `merge_when_matched_and_predicate_slot`, `merge_next_keyword` have zero call sites outside `engine.rs` (grepped `dsl-completion/src/*.rs` and `tests/*.rs`).
- `extract_first_ident`/`nth_ident` are only called by the other 7 functions in this same cluster -- stay private (no `pub`) in the new module.
- The other 7 are called only from `route_phase` (lines ~1166-1865), as bare function calls inside its if-chain.
- Both functions the cluster depends on from `engine.rs` itself -- `stmt_slice_upper` (line 34) and `cursor_not_at_ws_boundary` (line 46) -- are currently private `fn`; need `pub(crate)` to be reachable from the new module.

## Task 1: Extract `merge.rs`

**Files:**
- Create: `dsl-completion/src/merge.rs`
- Modify: `dsl-completion/src/lib.rs` (add `pub mod merge;`)
- Modify: `dsl-completion/src/engine.rs` (remove the 9 function bodies from lines 8981-9239; widen `stmt_slice_upper`/`cursor_not_at_ws_boundary` to `pub(crate)`; add `use crate::merge;`; prefix the 7 call sites with `merge::`)

- [ ] **Step 1: Widen the two shared helpers' visibility in `engine.rs`**

Change (lines 34 and 46):
```rust
...
- [ ] **Step 3: Register the module and remove the old code from `engine.rs`**

In `lib.rs`, add `pub mod merge;` alongside the other `pub mod` lines.

In `engine.rs`: delete lines 8981-9239 (the 9 function bodies, now in `merge.rs`); add `use crate::merge;` near the top (with the other `use crate::X;` lines); update the 7 call sites (lines ~1166, 1170, 1174, 1180, 1832, 1835, 1839-1840, 1846, 1853, 1865) to prefix with `merge::` (`merge::merge_insert_col_list_slot(...)`, `merge::merge_target_and_source(...)`, `merge::merge_update_set_lhs_slot(...)`, `merge::merge_when_matched_and_predicate_slot(...)`, `merge::merge_update_set_rhs_expr_slot(...)`, `merge::merge_aliases(...)`, `merge::merge_next_keyword(...)`).

- [ ] **Step 4: Compile and run the full test suite**

Run: `cargo build -p dsl-completion --release 2>&1 | tail -40`
Expected: compiles clean (fixes any missed call-site reference).

Run: `cargo test -p dsl-completion --release 2>&1 | grep "test result:"`
Expected: identical pass counts to the pre-batch baseline (5, 9, 2031, 5, 0 passed/2 ignored, 23, 0) -- 0 failed anywhere. Any difference means behavior changed, which this batch must not do.
...
```bash
git add dsl-completion/src/merge.rs dsl-completion/src/lib.rs dsl-completion/src/engine.rs \
        dsl-completion/docs/plans/2026-08-18-phase-b-batch-1-merge-module.md
git commit -m "refactor(completion): extract MERGE completion into merge.rs

Phase B batch 1 (of the engine.rs incremental refactor) -- pure code
motion, zero logic change. Moves the 9-function MERGE statement
completion cluster out of engine.rs into its own module, matching the
existing create_index.rs/create_table.rs/sources.rs sibling-module
convention. engine.rs: 10925 -> ~10685 lines."
```
