# duck-gate — context dump for Claude

Paste this into the system / project memory of any Claude session to bring it up to speed on duck-gate.

---

## What it is

**duck-gate** is a native Rust API gateway. Sits between clients and backend services. Routes, terminates TLS, authenticates, rate-limits, caches, breaks circuits, observes. Hyper + Tokio + rustls under the hood.

CLI binary name: `duck-gate`. Top-level Rust crate: `dgw`. Repo: `github.com/gentleeduck/duck-gate`.
...
Gap duck-gate fills: **single-binary, Rust-fast, self-hostable, JS-extendable** gateway with Vercel-grade DX.
...
- `duck-gate` — **this project**
- `duck-ttlog` — lock-free Rust logger w/ crash snapshots (separate concern; **not** the tracing layer for duck-gate)
...
```sh
duck-gate run --config duck-gate.toml
```

## CLI shape

```
duck-gate run    --config FILE [--watch]   # boot the server
duck-gate check  --config FILE             # validate config
duck-gate reload --admin URL               # POST to admin to hot-reload
```
...
- duck-gate uses `tracing` + OpenTelemetry for distributed tracing. **NOT** ttlog. ttlog is a separate sibling project for hot-path lock-free logging + crash snapshots.
- "dgw" is the crate prefix. CLI binary is "duck-gate".
