...
pub fn statement(s: sp::Statement, raw: &str) -> StatementKind {
    ...
    match s {
        ...
        S::AlterTable { name, .. } => StatementKind::AlterTable(AlterTableStmt {
        ...
        S::Drop {
            ...
            names,
            ..
        } => StatementKind::DropTable(DropTableStmt {
            if_exists,
            tables: names.iter().map(object_name).collect(),
        }),
...
fn projection(p: &sp::SelectItem, raw: &str) -> Projection {
    match p {
        ...
        sp::SelectItem::UnnamedExpr(e) => Projection::Expr {
            expr: expr(e, raw),
            alias: None,
        },
        sp::SelectItem::ExprWithAlias { expr: e, alias } => Projection::Expr {
            expr: expr(e, raw),
...
fn column_def(c: &sp::ColumnDef) -> ColumnDef {
    let not_null = c.options.iter().any(|o| {
        matches!(
            o.option,
            sp::ColumnOption::NotNull
                | sp::ColumnOption::Unique { is_primary: true, .. }
        )
    });
...
fn expr(e: &sp::Expr, _raw: &str) -> Expr {
    match e {
        sp::Expr::Identifier(id) => Expr::Column {
            qualifier: None,
            name: id.value.clone(),
            range: TextRange::default(),
        },
