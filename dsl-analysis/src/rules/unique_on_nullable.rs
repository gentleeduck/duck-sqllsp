    ...
    fn check(
        ...
        while i + 6 <= n {
                ...
                && (i + 6 == n || !is_word(bytes[i + 6] as char))
            {
                ...
                if after_kw.trim_start().starts_with("NULLS NOT DISTINCT") { i += 6; continue; }
                ...
                if is_inline_form {
                    if col_text.contains("NOT NULL") { i += 6; continue; }
                ...
                if j < n && bytes[j] == b'(' {
                    ...
                    while m < n && depth > 0 {
                        match bytes[m] { b'(' => depth += 1, b')' => depth -= 1, _ => {} }
                        if depth == 0 { break; }
                        m += 1;
                    }
                    ...
                    let any_nullable = list_text.split(',').any(|raw| {
                        ...
                        cols.iter().any(|(name, nullable)|
                            name.eq_ignore_ascii_case(&col) && *nullable
                        )
                    });
                    if any_nullable {
                        let abs_start = start + i;
        ...
        if c == b',' && d == 0 {
                ...
                && !chunk_up.trim_start().starts_with("LIKE")
            {
                let name: String = trimmed.chars()
                    ...
                    .collect();
                let nullable = !chunk_up.contains("NOT NULL")
