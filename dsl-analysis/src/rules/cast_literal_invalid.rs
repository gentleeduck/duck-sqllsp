...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    while let Some(rel) = upper[from..].find("CAST(") {
      ...
      let Some(as_at) = inner_upper.find(" AS ") else { from = inner_start + close; continue };
      ...
      if let Some(lit) = lit_part.strip_prefix('\'').and_then(|s| s.strip_suffix('\'')) {
        if let Some(reason) = literal_cast_error(lit, ty_part) {
          out.push(Diagnostic {
            code: "sql195",
            severity: Severity::Error,
            message: format!("CAST('{lit}' AS {ty_part}) -- {reason} (PG 22P02 at runtime)"),
            range: text_size::TextRange::new(((start + at) as u32).into(), ((start + inner_start + close + 1) as u32).into()),
          });
        }
      }
...
fn literal_cast_error(lit: &str, ty: &str) -> Option<String> {
  ...
  match bare.as_str() {
    "int" | "integer" | "int4" | "int8" | "int2" | "bigint" | "smallint" => {
      if lit.parse::<i64>().is_err() { return Some(format!("'{lit}' not a valid integer")); }
    }
    "numeric" | "decimal" | "real" | "double" | "float" | "float4" | "float8" => {
      if lit.parse::<f64>().is_err() { return Some(format!("'{lit}' not a valid number")); }
    }
    "boolean" | "bool" => {
      let v = lit.trim().to_ascii_lowercase();
      let ok = ["t", "f", "true", "false", "y", "n", "yes", "no", "1", "0", "on", "off"].contains(&v.as_str());
      if !ok { return Some(format!("'{lit}' not a valid boolean")); }
    }
    "uuid" => {
      let v = lit.trim().to_ascii_lowercase();
      ...
      if !ok { return Some(format!("'{lit}' not a valid uuid (need 8-4-4-4-12 hex)")); }
    }
    "date" => {
      let v = lit.trim();
      let segs: Vec<&str> = v.split('-').collect();
      let ok = segs.len() == 3
        && segs[0].len() == 4 && segs[0].chars().all(|c| c.is_ascii_digit())
        && segs[1].len() == 2 && segs[1].chars().all(|c| c.is_ascii_digit())
        && segs[2].len() == 2 && segs[2].chars().all(|c| c.is_ascii_digit());
      if !ok { return Some(format!("'{lit}' not a valid DATE (need YYYY-MM-DD)")); }
    }
    _ => {}
  }
  None
