...
fn count_kw_excluding(s: &str, needle: &str, excluded: &[&str]) -> usize {
  ...
  while let Some(rel) = s[from..].find(needle) {
    ...
    let after_ok = after_pos >= bytes.len() || !{ let p = bytes[after_pos] as char; p.is_ascii_alphanumeric() || p == '_' };
    if before_ok && after_ok {
      ...
      let is_excluded = excluded.iter().any(|ex| {
        let elen = ex.len();
        after_text.len() >= elen && after_text[..elen].eq_ignore_ascii_case(ex)
          && (after_text.len() == elen || !{ let p = after_text.as_bytes()[elen] as char; p.is_ascii_alphanumeric() || p == '_' })
      });
      if !is_excluded { n += 1 }
    }
    from = after_pos;
