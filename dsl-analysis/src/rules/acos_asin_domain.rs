//! sql694: `acos(2)` / `asin(-3)` -- the argument to acos/asin is a literal
//! outside the valid domain [-1, 1]. PostgreSQL raises 2201E ("input is out of
//! range") at runtime. Almost always a typo or a value that should have been
//! normalised first. (Companion to sql690 sqrt_negative_literal and sql692
//! ln_log_nonpositive_literal.)
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let mut i = 0usize;
    while i < n {
      if !word_at(ub, i, b"ACOS") && !word_at(ub, i, b"ASIN") {
        i += 1;
        continue;
      }
      ...
      if let Some((s, e)) = trim_range(ub, p + 1, close)
        && top_level_args(ub, p, close).len() == 1
        && upper[s..e].parse::<f64>().is_ok_and(|v| !(-1.0..=1.0).contains(&v))
      {
        out.push(Diagnostic {
          code: "sql694",
          severity: Severity::Warning,
          message: "acos/asin argument is outside the valid domain [-1, 1] -- raises an error at runtime".into(),
