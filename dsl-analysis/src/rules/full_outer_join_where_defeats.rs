//! sql789: `a FULL [OUTER] JOIN b ON ... WHERE b.col = 'x'` -- a
//! positive WHERE predicate on either side of a FULL OUTER JOIN
//! silently turns it into a non-full join: the NULL-extended rows fail
//! the filter and disappear. Sibling to the existing
//! left_join_defeated_by_where (sql522), extended to check both sides
//! since both are nullable in a FULL JOIN. Same conservative rules:
//! only a conjunct that *begins* with `alias.col <predicate>` is
//! flagged, and any conjunct mentioning NULL or containing a top-level
//! OR is skipped.
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let aliases = full_join_aliases(ub, body);
    if aliases.is_empty() {
      return;
    }
    ...
    for (s, e) in split_on_and(body, pred_start, pred_end) {
      ...
      out.push(Diagnostic {
        ...
        message: format!(
          "`{alias}` is a nullable side of a FULL JOIN, but this WHERE filter on it forces a non-full join -- move the condition into the ON clause"
        ),
...
/// Aliases (lowercased) of BOTH sides of every `FULL [OUTER] JOIN`:
/// the identifier right before this join's own `FULL` keyword (left
/// side) and the identifier right before its `ON` (right side).
fn full_join_aliases(ub: &[u8], body: &str) -> Vec<String> {
  let n = ub.len();
  let mut out = Vec::new();
  let mut i = 0usize;
  while i < n {
    if !word_at(ub, i, b"FULL") {
      i += 1;
      continue;
    }
    if let Some(left) = ident_before(body, i) {
      out.push(left.to_ascii_lowercase());
    }
