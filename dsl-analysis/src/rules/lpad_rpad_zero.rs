//! sql699: `lpad(s, 0)` / `rpad(s, 0)` -- padding (or truncating) a string to
//! length 0 always yields the empty string, so the call is a constant `''`.
//! Almost always a typo for a real width. (Companion to sql448
//! lpad_rpad_negative, which covers negative lengths, and sql679
//! left_right_zero.)
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    while i < n {
      ...
      // lpad/rpad take (string, length [, fill]); flag length == 0.
      if args.len() >= 2
        ...
        && &upper[s..e] == "0"
      {
        out.push(Diagnostic {
          ...
          message: "lpad()/rpad() to length 0 is always the empty string -- check the width".into(),
