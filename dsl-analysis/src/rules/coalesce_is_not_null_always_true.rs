//! sql710: `COALESCE(x, 0) IS NOT NULL` -- when COALESCE's last argument is a
//! non-NULL constant the result can never be NULL, so `IS NOT NULL` is always
//! true and the predicate matches every row. Almost always a logic slip (the
//! guard does nothing). (Mirror of sql702 coalesce_is_null_always_false.)
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    while i < n {
      ...
      if word_at(ub, q, b"IS") {
        q = skip_ws(ub, q + 2);
        if word_at(ub, q, b"NOT") {
          let r = skip_ws(ub, q + 3);
          if word_at(ub, r, b"NULL") {
              ...
              && is_nonnull_literal(&upper[s..e])
            {
              out.push(Diagnostic {
                ...
                message: "COALESCE with a non-NULL fallback is never NULL -- this IS NOT NULL is always true".into(),
                range: crate::range_at(start + i, start + r + 4),
              });
            }
          }
        }
