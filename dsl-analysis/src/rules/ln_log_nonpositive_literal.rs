//! sql692: `ln(0)` / `ln(-1)` / `log(0)` / `log(-5)` -- the logarithm of a
//! non-positive literal. PostgreSQL raises 2201E ("cannot take logarithm of a
//! negative number" / "... of zero") at runtime. Almost always a sign typo or
//! a placeholder. (Companion to sql690 sqrt_negative_literal. The two-argument
//! `log(base, x)` form is handled by sql693.)
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let mut i = 0usize;
    while i < n {
      let fname = if word_at(ub, i, b"LN") {
        2
      } else if word_at(ub, i, b"LOG") {
        3
      } else {
        i += 1;
        continue;
      ...
      // Single-argument ln()/log(); the two-arg log(base, x) is sql693.
      if args.len() == 1
        && let Some((s, e)) = trim_range(ub, args[0].0, args[0].1)
        && parse_num(&upper[s..e]).is_some_and(|v| v <= 0.0)
      {
        out.push(Diagnostic {
          code: "sql692",
          severity: Severity::Warning,
          message: "logarithm of a non-positive number raises an error at runtime (PG 2201E)".into(),
