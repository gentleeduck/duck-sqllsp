...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql138" }
    ...
    fn check(
        ...
        if upper[dist..].starts_with("DISTINCT ON") { return; }
        let from_at = upper.find(" FROM ").unwrap_or(upper.len());
        if dist >= from_at { return; }
        let proj = &upper[dist + 8..from_at];
        if !proj.contains("::TEXT") { return; }
        ...
        out.push(Diagnostic {
            ...
            message: "::text cast inside DISTINCT throws away typed comparison -- DISTINCT runs string compare on the cast result".into(),
