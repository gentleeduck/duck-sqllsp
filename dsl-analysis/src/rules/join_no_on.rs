...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql064" }
    ...
    fn check(
        ...
        while i + 4 <= n {
            ...
            {
                let mut p = i;
                while p > 0 && (bytes[p - 1] as char).is_whitespace() { p -= 1; }
                let end_w = p;
                while p > 0 && is_word(bytes[p - 1] as char) { p -= 1; }
                ...
                while j < n {
                    ...
                    if depth == 0 && c.is_ascii_alphabetic() {
                        ...
                        if word == "ON" || word == "USING" { has_on = true; break; }
                        if matches!(word, "JOIN" | "WHERE" | "GROUP" | "ORDER" | "LIMIT" | "OFFSET"
                            | "UNION" | "EXCEPT" | "INTERSECT" | "RETURNING" | "HAVING" | "WINDOW") {
                            break;
                        }
                        continue;
                    }
