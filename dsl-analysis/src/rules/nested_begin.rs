...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let closes = count_kw(&prelude_upper, "COMMIT")
      + count_kw(&prelude_upper, "ROLLBACK")
      + count_phrase(&prelude_upper, "END TRANSACTION");
    if begins <= closes {
      return;
    }
    ...
    out.push(Diagnostic {
      ...
      message:
        "Nested BEGIN -- a transaction is already open; PG warns and ignores. For a rollback-only unit use SAVEPOINT"
          .into(),
