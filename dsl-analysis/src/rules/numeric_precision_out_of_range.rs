//! sql724: `NUMERIC(2000)` / `DECIMAL(0, 0)` -- the precision must be between
//! 1 and 1000. PostgreSQL rejects a precision outside that range with 22023
//! ("NUMERIC precision N must be between 1 and 1000"). Usually a typo or a
//! confusion with the column's intended length. (Companion to sql450
//! numeric_scale_exceeds_precision.)
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let mut i = 0usize;
    while i < n {
      if !word_at(ub, i, b"NUMERIC") && !word_at(ub, i, b"DECIMAL") {
        i += 1;
        continue;
      }
      ...
      if let Some(first) = args.first()
        && let Some((s, e)) = trim_range(ub, first.0, first.1)
        && upper[s..e].parse::<i64>().is_ok_and(|v| !(1..=1000).contains(&v))
      {
        out.push(Diagnostic {
          code: "sql724",
          severity: Severity::Warning,
          message: "NUMERIC/DECIMAL precision must be between 1 and 1000 (PG 22023)".into(),
