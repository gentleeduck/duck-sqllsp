//! sql715: `starts_with(x, '')` -- every string starts with the empty string,
//! so this is always true (NULL when `x` is NULL). Almost always a leftover
//! placeholder or a prefix that was never filled in. (Companion to sql716
//! translate_empty_from.)
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let mut i = 0usize;
    while i < n {
      if !word_at(ub, i, b"STARTS_WITH") {
        i += 1;
        continue;
      }
      let p = skip_ws(ub, i + 11);
      if ub.get(p) != Some(&b'(') {
        i += 11;
        continue;
      }
        ...
        && &upper[s..e] == "''"
      {
        out.push(Diagnostic {
          ...
          message: "starts_with(x, '') is always true -- every string starts with the empty string".into(),
