//! sql704: `nth_value(x, 0)` / `nth_value(x, -1)` -- the position must be
//! positive. PostgreSQL raises 22023 ("argument of nth_value must be greater
//! than zero") at runtime. Positions are 1-based; `nth_value(x, 1)` is the
//! first row of the frame. (Companion to sql703 ntile_nonpositive.)
    ...
    while i < n {
        ...
        && upper[s..e].parse::<f64>().is_ok_and(|v| v <= 0.0)
      {
        out.push(Diagnostic {
          ...
          message: "nth_value() position must be greater than zero -- raises an error at runtime (PG 22023)".into(),
