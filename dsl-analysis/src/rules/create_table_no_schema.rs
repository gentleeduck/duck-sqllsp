...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    if !buffer_has_qualified_create(source) {
      return;
    }
    out.push(Diagnostic {
      code: "sql327",
      ...
      message: format!(
        "CREATE TABLE `{}` has no schema qualifier -- other tables in this buffer ARE qualified; pick one style",
        ct.table.name
      ),
      range: stmt.range,
    });
