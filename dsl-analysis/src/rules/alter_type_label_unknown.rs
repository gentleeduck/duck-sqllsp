...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    if !upper.contains("ALTER TYPE") { return }
    ...
    let id_end = name_rest.find(|c: char| !c.is_ascii_alphanumeric() && c != '_' && c != '.' && c != '"').unwrap_or(name_rest.len());
    ...
    if labels.iter().any(|l| l.eq_ignore_ascii_case(anchor)) { return }
    ...
    out.push(Diagnostic {
      ...
      message: format!(
        "{kw} `{anchor}` is not a label of enum `{type_name}` -- valid labels: {}",
        labels.join(", "),
      ),
  ...
  while let Some(rel) = upper[from..].find(needle) {
    ...
    if n.eq_ignore_ascii_case(name) {
      ...
      if let Some(enum_at) = body_upper.find("AS ENUM") {
        ...
        if !post.starts_with('(') { return Vec::new() }
