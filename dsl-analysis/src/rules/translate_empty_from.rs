//! sql716: `translate(s, '', to)` -- an empty `from` set means there are no
//! characters to map, so translate returns `s` unchanged. The call is a no-op,
//! almost always a leftover or a swapped argument. (Companion to sql467
//! empty_needle_string_fn for replace/split_part.)
    ...
    while i < n {
        ...
        && &upper[s..e] == "''"
      {
        out.push(Diagnostic {
          ...
          message: "translate() with an empty `from` set is a no-op -- it returns the string unchanged".into(),
