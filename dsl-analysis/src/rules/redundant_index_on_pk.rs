...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql167" }
    ...
    fn check(
        ...
        let is_index = trimmed.starts_with("CREATE INDEX")
        ...
        if pk.columns.len() != 1 { return; }
        if !pk.columns[0].eq_ignore_ascii_case(&idx_col) { return; }
        let abs_start = start;
