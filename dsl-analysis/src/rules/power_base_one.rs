//! sql685: `power(1, x)` always returns 1 -- 1 raised to any exponent is 1.
//! The call is a constant, almost always a typo for a real base or a leftover
//! placeholder. (Companion to sql447 power_trivial_exponent, which covers the
//! `power(x, 0)` / `power(x, 1)` exponent side.)
    ...
    while i < n {
        ...
        && &upper[s..e] == "1"
      {
        out.push(Diagnostic {
          ...
          message: "power(1, x) is always 1 -- check the base".into(),
