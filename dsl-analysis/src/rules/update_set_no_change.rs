...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql149" }
    ...
    fn check(
        ...
        let end_at = upper[after_set..].find(" WHERE ")
            .map(|p| after_set + p)
            .unwrap_or(end - start);
        ...
        while idx <= n {
            ...
            match c {
                b'(' => { depth += 1; idx += 1; continue; }
                b')' => { depth -= 1; idx += 1; continue; }
                b'\'' if !at_end => {
                    idx += 1;
                    while idx < n && bytes[idx] != b'\'' { idx += 1; }
                    if idx < n { idx += 1; }
                    continue;
                }
                _ => {}
            ...
            if c == b',' && depth == 0 {
                ...
                if let Some(eq_at) = assign.find('=') {
                    ...
                    if !lhs.is_empty() && lhs.eq_ignore_ascii_case(rhs) {
                        ...
                        let abs_end = start + after_set + start_a + assign.trim_end().len() + (assign.len() - assign.trim_start().len());
                        out.push(Diagnostic {
                            code: "sql149",
                            severity: Severity::Hint,
                            message: format!("`{lhs} = {rhs}` is a no-op assignment -- the row is rewritten and triggers fire for nothing"),
