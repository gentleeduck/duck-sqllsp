...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    out.push(Diagnostic {
      ...
      message: format!("table `{}` has no PRIMARY KEY -- add one or mark as a log/audit table", ct.table.name,),
      range,
    });
  }
}

fn find_table_name_range(body: &str, body_offset: usize, name: &str) -> Option<text_size::TextRange> {
  let upper = body.to_ascii_uppercase();
