//! sql748: `encode(data, 'base32')` -- the format (second argument) must be
//! one of `base64`, `hex`, or `escape`. PostgreSQL raises 22023 ("unrecognized
//! encoding") at runtime for anything else. Usually a typo (`base32`, `b64`)
//! or a confusion with another language's API.
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    while i < n {
      ...
      if args.len() == 2
        ...
        && ub[e - 1] == b'\''
      {
        let fmt = body[s + 1..e - 1].to_ascii_lowercase();
        if !matches!(fmt.as_str(), "base64" | "hex" | "escape") {
          out.push(Diagnostic {
            code: "sql748",
            severity: Severity::Warning,
            message: format!("encode() format '{fmt}' is not recognized -- use 'base64', 'hex', or 'escape'"),
