...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql036" }
    ...
    fn check(
        ...
        while i + 5 <= n {
            if &upper_body[i..i + 5] == "RAISE" {
                ...
                if prev_ok && next_ok {
                    if let Some((placeholders, args, fmt_start, fmt_end)) =
                        parse_raise(raw, i)
                    {
                        if placeholders != args {
                            ...
                            out.push(Diagnostic {
                                ...
                                message: format!(
                                    "RAISE format has {placeholders} `%` placeholder(s) but {args} argument(s)"
                                ),
...
fn parse_raise(bytes: &[u8], raise_at: usize) -> Option<(usize, usize, usize, usize)> {
    ...
    while k < fmt.len() {
        if fmt[k] == b'%' {
            if k + 1 < fmt.len() && fmt[k + 1] == b'%' { k += 2; continue; }
            placeholders += 1;
        }
        k += 1;
    }
    ...
    while i < n {
        ...
        if depth == 0 && c.is_ascii_alphabetic() {
            ...
            if i + 5 <= n && bytes[i..i + 5].eq_ignore_ascii_case(b"USING")
