...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, scope: &Scope, catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let mut valid_cols: std::collections::HashSet<String> =
    ...
    while i < bytes.len() {
      ...
      if is_keyword(&upper_tok) {
        continue;
      }
      if valid_cols.contains(&token.to_ascii_lowercase()) {
        continue;
      }
      ...
      if catalog.tables().any(|tb| tb.name.eq_ignore_ascii_case(token)) {
        continue;
      }
      // Token may be a CTE name or alias the scope knows about.
      if scope.get(token).is_some() {
        continue;
      }
