...
const VOLATILE: &[&str] = &[
  ...
  "session_user", "current_user", "current_role",
];
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    if !matches!(&stmt.kind, StatementKind::CreateTable(_)) { return }
    ...
    while let Some(rel) = upper[from..].find("GENERATED ALWAYS AS") {
      ...
      let Some(close) = find_matching_paren(body, abs_open) else { from = after; continue };
      let expr = &body[abs_open + 1..close];
      let trailing = body[close + 1..].trim_start();
      if !trailing.to_ascii_uppercase().starts_with("STORED") { from = close; continue }
      ...
      for v in VOLATILE {
        ...
        if expr_lc.contains(&needle) {
          out.push(Diagnostic {
            ...
            range: text_size::TextRange::new((start as u32 + at as u32).into(), (start as u32 + close as u32 + 1).into()),
          });
          break;
        }
      }
