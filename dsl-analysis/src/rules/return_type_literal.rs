...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql031" }
                    ...
                    if let Some(kind) = classify_literal(raw_val) {
                        if !compatible(kind, declared.as_str()) {
                            ...
                            out.push(Diagnostic {
                                ...
                                message: format!(
                                    "RETURN value type {} doesn't match declared RETURNS {}",
                                    kind_name(kind), declared
                                ),
...
fn classify_literal(s: &str) -> Option<LitKind> {
    ...
    if upper == "NULL" { return Some(LitKind::Null); }
    if upper == "TRUE" || upper == "FALSE" { return Some(LitKind::Bool); }
...
fn compatible(kind: LitKind, declared_upper: &str) -> bool {
    ...
    if matches!(kind, LitKind::Unknown | LitKind::Null) { return true; }
...
fn find_returns_type(upper: &str) -> Option<String> {
    ...
    for c in trimmed.chars() {
        if paren == 0 && c.is_whitespace() { break; }
        if c == '(' { paren += 1; out.push(c); continue; }
        if c == ')' { paren -= 1; out.push(c); continue; }
        out.push(c);
    }
