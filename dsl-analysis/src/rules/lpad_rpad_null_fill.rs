//! sql708: `lpad(s, n, NULL)` / `rpad(s, n, NULL)` -- a NULL fill string makes
//! the whole result NULL whenever padding is actually needed (the input is
//! shorter than `n`), which is almost never intended. Use a real pad string
//! (the default is a space). (Companion to sql699 lpad_rpad_zero.)
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let mut i = 0usize;
    while i < n {
      if !word_at(ub, i, b"LPAD") && !word_at(ub, i, b"RPAD") {
        i += 1;
        continue;
      }
      ...
      if args.len() == 3
        && let Some((s, e)) = trim_range(ub, args[2].0, args[2].1)
        && &upper[s..e] == "NULL"
      {
        out.push(Diagnostic {
          code: "sql708",
