...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let end_at = after.find("COMMIT").or_else(|| after.find("ROLLBACK")).unwrap_or(after.len());
    ...
    if tx_body.contains("LOCK TABLE")
      || tx_body.contains("FOR UPDATE")
      || tx_body.contains("FOR SHARE")
      || tx_body.contains("FOR NO KEY UPDATE")
    {
      return;
    }
