...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    while let Some(rel) = upper[from..].find("INTERVAL") {
      ...
      if at >= 2 && &body[at - 2..at] == "::" { from = after; continue }
      // Find next number then unit keyword.
      let bytes_post = post.as_bytes();
      let mut k = 0usize;
      // optional sign
      if k < bytes_post.len() && (bytes_post[k] == b'-' || bytes_post[k] == b'+') { k += 1 }
      let num_start = k;
      while k < bytes_post.len() && (bytes_post[k].is_ascii_digit() || bytes_post[k] == b'.') { k += 1 }
      if k == num_start { from = after; continue }
      while k < bytes_post.len() && bytes_post[k].is_ascii_whitespace() { k += 1 }
      let unit_start = k;
      while k < bytes_post.len() && bytes_post[k].is_ascii_alphabetic() { k += 1 }
      let unit: String = post[unit_start..k].to_ascii_uppercase();
      if !UNITS.contains(&unit.as_str()) { from = after; continue }
      let abs_s = start + at;
      ...
      out.push(Diagnostic {
        ...
        message: format!(
          "`INTERVAL` literal without quotes -- MySQL syntax; PG requires `INTERVAL '<n> <unit>'` like `INTERVAL '1 day'`"
        ),
