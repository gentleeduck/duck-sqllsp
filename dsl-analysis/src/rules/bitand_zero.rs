//! sql713: `x & 0` / `0 & x` -- a bitwise AND with 0 is always 0 (NULL when
//! `x` is NULL), so the expression is a constant. Almost always a typo (a
//! different mask was meant) or a disabled term. (Companion to sql681
//! multiply_by_zero.)
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    while i < n {
      match ub[i] {
        ...
        // Single `&` (bitwise AND), not `&&` (array overlap).
...
fn diag(s: usize, e: usize) -> Diagnostic {
  Diagnostic {
    ...
    message: "bitwise AND with 0 is always 0 -- check the mask".into(),
