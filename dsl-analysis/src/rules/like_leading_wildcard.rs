...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql088" }
    ...
    fn check(
        ...
        while i + 5 <= n {
            ...
            if here.starts_with(b"ILIKE") && (i == 0 || !is_word(bytes[i - 1] as char))
                && (i + 5 == n || !is_word(bytes[i + 5] as char)) {
                kw_len = 5;
            } else if here.starts_with(b"LIKE") && (i == 0 || !is_word(bytes[i - 1] as char))
            ...
            if j < n && bytes[j] == b'\'' {
                if j + 1 < n && bytes[j + 1] == b'%' {
                    ...
                    out.push(Diagnostic {
                        ...
                        message: "LIKE/ILIKE with leading `%` prevents B-tree index use -- consider pg_trgm or full-text search".into(),
