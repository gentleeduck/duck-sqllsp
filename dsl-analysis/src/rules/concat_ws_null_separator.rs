//! sql688: `concat_ws(NULL, a, b)` -- a NULL separator makes the whole result
//! NULL. Unlike the value arguments (which concat_ws happily skips when NULL),
//! a NULL *separator* short-circuits the entire call to NULL, which is almost
//! never intended. Use a real separator (e.g. `''` for no separator).
//! (Companion to sql465 concat_ws_empty_sep.)
    ...
    while i < n {
        ...
        && &upper[s..e] == "NULL"
      {
        out.push(Diagnostic {
          ...
          message: "concat_ws() with a NULL separator returns NULL for the whole call -- use a real separator".into(),
