//! sql712: `make_time(25, 0, 0)` -- an hour, minute or second literal outside
//! its valid range. `make_time(hour, min, sec)` needs hour in 0..23, min in
//! 0..59 and sec in 0..<60; PostgreSQL raises 22008 ("time field value out of
//! range") at runtime. Usually an off-by-one or a transposed field.
//! (Companion to sql711 make_date_invalid.)
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    while i < n {
      ...
      if args.len() == 3 {
        flag_if(ub, &upper, args[0], |v| !(0.0..=23.0).contains(&v), "hour", start, out);
        flag_if(ub, &upper, args[1], |v| !(0.0..=59.0).contains(&v), "minute", start, out);
        flag_if(ub, &upper, args[2], |v| !(0.0..60.0).contains(&v), "second", start, out);
      }
      i = close + 1;
    }
  }
...
fn flag_if(ub: &[u8], upper: &str, arg: (usize, usize), bad: impl Fn(f64) -> bool, field: &str, start: usize, out: &mut Vec<Diagnostic>) {
    ...
    && upper[s..e].parse::<f64>().is_ok_and(bad)
  {
    out.push(Diagnostic {
      ...
      message: format!("make_time() {field} is out of range -- raises a time error at runtime (PG 22008)"),
