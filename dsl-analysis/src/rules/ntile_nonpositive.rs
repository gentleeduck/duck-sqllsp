//! sql703: `ntile(0)` / `ntile(-2)` -- the bucket count must be positive.
//! PostgreSQL raises 22023 ("argument of ntile must be greater than zero") at
//! runtime for a literal <= 0. Usually a typo or an uninitialised variable
//! that resolved to 0. (Companion to sql704 nth_value_nonpositive.)
    ...
    while i < n {
        ...
        && upper[s..e].parse::<f64>().is_ok_and(|v| v <= 0.0)
      {
        out.push(Diagnostic {
          ...
          message: "ntile() bucket count must be greater than zero -- raises an error at runtime (PG 22023)".into(),
