...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    if upper.contains("BTREE_GIST") || upper.contains("OPCLASS") || upper.contains("INT4_OPS") {
      return;
    }
    ...
    for raw in cols.split(',') {
      ...
      let ok = ty.starts_with("tsvector")
        || ty.starts_with("tsquery")
        || ty.contains("range")
        || ty.contains("multirange")
        || ty.contains("geometry")
        || ty.contains("geography")
        || ty.contains("point")
        || ty.contains("box")
        || ty.contains("circle")
        || ty.contains("polygon")
