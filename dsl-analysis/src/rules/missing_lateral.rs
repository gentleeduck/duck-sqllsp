...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql151" }
        ...
        while i < n {
            match bytes[i] {
                ...
                b',' if depth == 0 => {
                    ...
                    let id_start = j;
                    while j < n && (is_word(bytes[j] as char)) { j += 1; }
                    if j == id_start { i += 1; continue; }
                    ...
                    if inner.contains('.') {
                        ...
                        let has_lateral = before.rsplit_once("LATERAL")
