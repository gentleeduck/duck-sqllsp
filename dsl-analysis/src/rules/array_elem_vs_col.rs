...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    if j >= bytes.len() || bytes[j] != b'(' { return }
    ...
    for (idx, raw_val) in elems.iter().enumerate() {
      ...
      if !val_upper.starts_with("ARRAY[") || !val_trim.ends_with(']') { continue }
      ...
      if col_fam != TypeFamily::Array { continue }
      let raw_ty = col.data_type.trim_end_matches("[]");
      let target_elem = classify(raw_ty);
      if target_elem == TypeFamily::Unknown { continue }
      if !families_compatible(elem_fam, target_elem) {
        ...
        out.push(Diagnostic {
          ...
          message: format!(
            ...
            elem_fam.name(), col.name, target_elem.name()
          ),
