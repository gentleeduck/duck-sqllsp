//! sql717: `to_char(x, '')` -- an empty format string always produces the
//! empty string, regardless of `x`. The call is a constant `''`, almost always
//! a placeholder where the real format pattern was never filled in.
    ...
    while i < n {
        ...
        && &upper[s..e] == "''"
      {
        out.push(Diagnostic {
          ...
          message: "to_char() with an empty format string always returns '' -- supply a real format".into(),
