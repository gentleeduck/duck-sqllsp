//! sql718: `repeat(s, 1)` -- repeating a string once returns it unchanged, so
//! the call is a no-op. Usually a leftover or a count that should have been
//! larger. (sql452 repeat_trivial_count covers the 0 / negative cases.)
    ...
    while i < n {
        ...
        && &upper[s..e] == "1"
      {
        out.push(Diagnostic {
          ...
          message: "repeat(s, 1) returns the string unchanged -- the call is a no-op".into(),
