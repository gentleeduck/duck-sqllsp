...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql124" }
    ...
    fn check(
        ...
        if !contains_word(&inner_up, &name.to_ascii_uppercase()) { return; }
        let abs_start = start + leading_ws;
        let abs_end = start + leading_ws + 4;
        out.push(Diagnostic {
            code: "sql124",
            severity: Severity::Hint,
            message: format!("CTE `{name}` self-references inside its body but `WITH` lacks `RECURSIVE` -- PG will reject the query"),
