//! sql693: `log(1, x)` -- a logarithm to base 1. PostgreSQL computes
//! `log(b, x)` as `ln(x) / ln(b)`, and `ln(1)` is 0, so base 1 raises a
//! division-by-zero error (22012) at runtime. Almost always a typo for a real
//! base (`log(10, x)`, `log(2, x)`). (Companion to sql692 for the single-arg
//! `log(x)` non-positive case.)
...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    while i < n {
      ...
      // log(base, x) -- base 1 is a division-by-zero.
      if args.len() == 2
        ...
        && &upper[s..e] == "1"
      {
        out.push(Diagnostic {
          ...
          message: "log() to base 1 is a division-by-zero error at runtime -- check the base".into(),
