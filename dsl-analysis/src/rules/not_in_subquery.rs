...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql018" }
    ...
    fn check(
        ...
        while let Some(rel) = upper[from..].find("NOT IN") {
            ...
            if !prev_ok { from = i + 1; continue; }
            // Skip whitespace then look for `(`
            let mut k = after;
            while k < slice.len() && (slice.as_bytes()[k] as char).is_whitespace() { k += 1; }
            if k >= slice.len() || slice.as_bytes()[k] != b'(' { from = after; continue; }
            // Skip whitespace inside paren
            let mut m = k + 1;
            while m < slice.len() && (slice.as_bytes()[m] as char).is_whitespace() { m += 1; }
            if upper[m..].starts_with("SELECT") {
                ...
                out.push(Diagnostic {
                    ...
                    message: "NOT IN (<subquery>) returns 0 rows when the subquery yields any NULL; \
                              prefer NOT EXISTS or add IS NOT NULL inside the subquery".into(),
