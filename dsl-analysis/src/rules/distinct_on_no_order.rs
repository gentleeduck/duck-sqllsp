...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql101" }
    ...
    fn check(
        ...
        let id_start = k;
        while k < n && (is_word(bytes[k] as char) || bytes[k] == b'.') { k += 1; }
        let id_end = k;
        if id_end == id_start { return; }
        ...
        let matches_order = if has_order {
            ...
            after.split(|c: char| c == ',' || c.is_ascii_whitespace())
                .next()
                .map(|tok| {
