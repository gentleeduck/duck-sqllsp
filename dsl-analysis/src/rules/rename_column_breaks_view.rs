...
fn find_views_referencing(source: &str, table: &str, old_col: &str) -> Vec<String> {
  ...
  let mut out = Vec::new();
  for needle in ["CREATE OR REPLACE VIEW ", "CREATE VIEW ", "CREATE MATERIALIZED VIEW ", "CREATE OR REPLACE MATERIALIZED VIEW "] {
    let mut from = 0usize;
    while let Some(rel) = upper[from..].find(needle) {
      ...
      if word_contains(body, table) && word_contains(body, old_col) {
        if !out.contains(&view_name) { out.push(view_name); }
      }
      from = stmt_end + 1;
    }
  }
