...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql141" }
    ...
    fn check(
        ...
        if !trimmed.starts_with("ALTER TYPE") { return; }
        if !trimmed.contains("ADD VALUE") { return; }
        // Inside a BEGIN..COMMIT window?
        ...
        out.push(Diagnostic {
            ...
            message: "ALTER TYPE ... ADD VALUE cannot run inside an explicit transaction -- run it outside BEGIN..COMMIT".into(),
