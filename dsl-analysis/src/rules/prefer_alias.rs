...
impl LintRule for Rule {
    ...
    fn check(
        ...
        // explicit alias.
        let aliases: Vec<(String, String)> = scope
            .tables()
            .filter(|b| b.alias != b.table.name)
            .map(|b| (b.table.name.clone(), b.alias.clone()))
            .collect();
        ...
        for (table, alias) in &aliases {
            for hit in find_qualified_uses(body, table) {
                out.push(Diagnostic {
                    ...
                    message: format!(
                        "use alias `{alias}` instead of `{table}` (declared in this statement)"
                    ),
