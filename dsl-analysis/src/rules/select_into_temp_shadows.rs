...
impl LintRule for Rule {
  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    let prefix = if rest_upper.starts_with("TEMP ") { "TEMP " }
      else if rest_upper.starts_with("TEMPORARY ") { "TEMPORARY " }
      else { return };
    let body_after_prefix = &rest[prefix.len()..].trim_start();
    let id_end = body_after_prefix.find(|c: char| !c.is_ascii_alphanumeric() && c != '_' && c != '.' && c != '"').unwrap_or(body_after_prefix.len());
