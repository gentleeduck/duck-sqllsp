  ...
  fn check(&self, source: &str, stmt: &Statement, _scope: &Scope, _catalog: &Catalog, out: &mut Vec<Diagnostic>) {
    ...
    for kw in after_kw_keywords {
      if let Some(p) = upper.find(kw) {
        ...
        let raw = &rest[lead..];
        let tok_end = raw.find(|c: char| !c.is_ascii_alphanumeric() && c != '_' && c != '.' && c != '"').unwrap_or(raw.len());
        target = raw[..tok_end].to_string();
        break;
      }
    }
    let lc = target.to_ascii_lowercase();
    let is_system = lc.starts_with("pg_") || lc.starts_with("information_schema.")
      || lc == "information_schema";
    if !is_system { return }
    if !concurrently && !upper.contains("SYSTEM ") { return }
    let abs_s = start;
    let abs_e = start + body.find(';').unwrap_or(body.len());
    let msg = if concurrently {
      format!("REINDEX CONCURRENTLY on `{target}` -- system catalogs cannot be reindexed concurrently; drop CONCURRENTLY")
    } else {
