...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql119" }
    ...
    fn check(
        ...
        if !trimmed.starts_with("SET TRANSACTION ISOLATION") { return; }
        ...
        let begin_at = match begin_at { Some(p) => p, None => return };
        ...
        for s in &stmts[1..] { // skip the BEGIN itself
            let t = s.trim().to_ascii_uppercase();
            if t.is_empty() { continue; }
            if t.starts_with("SET TRANSACTION") || t.starts_with("SET LOCAL") { continue; }
