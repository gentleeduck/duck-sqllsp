...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql146" }
        ...
        while i < n {
            for (kw, w) in candidates {
                    ...
                    && (i + w == n || !is_word(bytes[i + w] as char))
                {
                    ...
                    if j < n && bytes[j] == b'(' { i += w; continue; }
                    let abs_start = start + i;
                    let abs_end = start + i + w;
                    out.push(Diagnostic {
                        code: "sql146",
                        severity: Severity::Hint,
                        message: "unbounded VARCHAR / CHARACTER VARYING -- use TEXT for no cap, or spell the cap as `VARCHAR(n)`".into(),
