...
pub fn effective_columns(body: &str) -> Vec<EffectiveCol> {
    ...
    while idx <= ln {
        ...
        match c {
            b'(' => { d += 1; idx += 1; continue; }
            b')' => { d -= 1; idx += 1; continue; }
            b'\'' if !at_end => {
                idx += 1;
                while idx < ln && lb[idx] != b'\'' { idx += 1; }
                if idx < ln { idx += 1; }
                continue;
            }
            _ => {}
...
fn classify_entry(
    ...
    if trimmed_up.starts_with("FOREIGN") || trimmed_up.starts_with("CHECK")
        || trimmed_up.starts_with("EXCLUDE") || trimmed_up.starts_with("LIKE")
    {
        return;
    }
    // Column definition.
    ...
    let data_type: String = after_name.chars()
    ...
    let has_not_null = chunk_up.contains("NOT NULL")
        || chunk_up.contains("PRIMARY KEY")
        || is_serial
        || is_identity;
    let has_default = chunk_up.contains("DEFAULT")
        || is_serial
        || chunk_up.contains("GENERATED");
