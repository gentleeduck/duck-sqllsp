...
pub fn token_at(src: &str, offset: TextSize) -> Option<String> {
    ...
    while start > 0 {
        ...
        if is_token(c) { start -= 1; } else { break; }
    }
    let mut end = pos;
    while end < bytes.len() {
        let c = bytes[end] as char;
        if is_token(c) { end += 1; } else { break; }
    }
    if start == end { return None; }
    Some(src[start..end].to_string())
}

fn is_token(c: char) -> bool { c.is_alphanumeric() || c == '_' || c == '.' }
...
pub fn window_at(src: &str, offset: TextSize) -> Window {
    ...
    while cur_start > 0 && is_word(chars[cur_start - 1]) { cur_start -= 1; }
    let mut cur_end = pos;
    while cur_end < chars.len() && is_word(chars[cur_end]) { cur_end += 1; }
    ...
    let mut words: Vec<String> = prefix
        .iter()
...
fn back_word_start(chars: &[char], mut end: usize) -> usize {
    while end > 0 && is_word(chars[end - 1]) { end -= 1; }
    end
}
...
fn fwd_word_end(chars: &[char], mut start: usize) -> usize {
    while start < chars.len() && is_word(chars[start]) { start += 1; }
    start
}
