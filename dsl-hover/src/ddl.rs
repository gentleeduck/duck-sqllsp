...
pub fn column_decl_at(file: &ParsedFile, source: &str, offset: TextSize) -> Option<String> {
  ...
  for stmt in &file.statements {
    ...
    let entry = entries.into_iter().find(|(s, e)| cursor_in_body >= *s && cursor_in_body <= *e)?;
    ...
    let ident =
      piece.split(|c: char| c.is_whitespace() || c == '(' || c == ',').find(|s| !s.is_empty())?.trim_matches('"');
  ...
  while i < slice.len() {
    match bytes[i] as char {
      ...
      ')' => {
        depth -= 1;
        if depth == 0 {
          return Some(((open + 1) as u32, i as u32));
        }
      },
      _ => {},
    }
