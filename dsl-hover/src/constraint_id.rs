...
pub fn classify(token: &str) -> Option<Kind> {
    ...
    for (prefix, kind) in [
        ...
        if t.starts_with(prefix) { return Some(kind); }
    }
    None
}
...
fn find_in_catalog<'a>(token: &str, catalog: &'a Catalog) -> Option<Found<'a>> {
    for t in catalog.tables() {
        for c in &t.constraints {
            if c.name.eq_ignore_ascii_case(token) { return Some(Found::Constraint(t, c)); }
        }
        for i in &t.indexes {
            if i.name.eq_ignore_ascii_case(token) { return Some(Found::Index(t, i)); }
        }
        for tg in &t.triggers {
            if tg.name.eq_ignore_ascii_case(token) { return Some(Found::Trigger(t, tg)); }
        }
    }
    None
}
