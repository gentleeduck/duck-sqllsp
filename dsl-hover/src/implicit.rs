...
fn table_level_unique_includes(body: &str, col: &str) -> bool {
    for entry in split_top_level(body) {
        ...
        let is_uq = upper.starts_with("UNIQUE")
            || (upper.starts_with("CONSTRAINT") && upper.contains("UNIQUE")
...
fn table_level_fk(body: &str, col: &str) -> Option<ForeignKey> {
    for entry in split_top_level(body) {
        let upper = entry.to_ascii_uppercase();
        let is_fk = upper.contains("FOREIGN KEY")
            && (upper.starts_with("FOREIGN KEY") || upper.starts_with("CONSTRAINT"));
        if !is_fk { continue; }
        // FOREIGN KEY (col1, col2) REFERENCES tbl (rcol1, rcol2)
        let cols = extract_paren_names(entry);
        if !cols.iter().any(|c| c.eq_ignore_ascii_case(col)) { continue; }
...
fn table_level_check_count(body: &str) -> usize {
    split_top_level(body)
        ...
        .filter(|e| {
            ...
            u.starts_with("CHECK ")
                || u.starts_with("CHECK(")
                || (u.starts_with("CONSTRAINT") && u.contains("CHECK"))
        })
        .count()
}
...
fn extract_paren_names(entry: &str) -> Vec<String> {
    let open = match entry.find('(') { Some(i) => i, None => return Vec::new() };
    let close = match entry[open..].find(')') { Some(i) => open + i, None => return Vec::new() };
    entry[open + 1..close]
        .split(',')
