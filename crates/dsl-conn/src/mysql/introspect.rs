...
pub async fn run(pool: &MySqlPool, spec: &ConnectionSpec) -> Result<Catalog, DriverError> {
    ...
    for row in sqlx::query(&schemas_sql)
        .fetch_all(pool)
        .await
        .map_err(io_err)?
    {
    ...
    let mut con_acc: BTreeMap<(String, String, String), (ConstraintKind, Vec<String>, Option<ConstraintRef>)> = BTreeMap::new();
    ...
    for ((schema_name, _), table) in by_table {
        let schema = schemas.entry(schema_name.clone()).or_insert_with(|| Schema {
            name: schema_name,
            tables: Vec::new(),
        });
