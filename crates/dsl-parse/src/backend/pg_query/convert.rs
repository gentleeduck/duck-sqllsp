...
use crate::ast::{
    AlterTableStmt, ColumnDef, CreateTableStmt, DeleteStmt, DropTableStmt, Expr, InsertStmt,
    JoinClause, JoinKind, Projection, SelectStmt, StatementKind, TableRef, UpdateStmt,
};
...
fn create_table(stmt: &CreateStmt, _text: &str) -> StatementKind {
    ...
    StatementKind::CreateTable(CreateTableStmt {
        table,
        if_not_exists,
        columns,
    })
...
fn walk_join(
    ...
    match j.larg.as_ref().and_then(|n| n.node.as_ref()) {
        Some(PgNode::RangeVar(r)) => {
            if from.is_empty() { from.push(rangevar_to_tableref(r)); }
        }
...
fn column_ref_to_projection(cref: &pg_query::protobuf::ColumnRef, alias: Option<String>) -> Projection {
    ...
    let range = if cref.location >= 0 {
        ...
        let len = parts.iter().map(|s| s.len() as u32).sum::<u32>()
            + parts.len().saturating_sub(1) as u32; // dots
    ...
    match parts.as_slice() {
        ...
        [name] => Projection::Expr {
            expr: Expr::Column { qualifier: None, name: name.clone(), range },
            alias,
        },
        [q, name] => Projection::Expr {
            expr: Expr::Column { qualifier: Some(q.clone()), name: name.clone(), range },
            alias,
        },
        _ => Projection::Expr { expr: Expr::Other(String::new()), alias },
    }
...
fn update(u: &pg_query::protobuf::UpdateStmt, _text: &str) -> UpdateStmt {
    ...
    let assignments = u
        ...
        .iter()
        .filter_map(|n| match n.node.as_ref()? {
            PgNode::ResTarget(t) if !t.name.is_empty() => {
                Some((t.name.clone(), Expr::Other(String::new())))
            }
            _ => None,
        })
        .collect();
    UpdateStmt {
        table,
        assignments,
        where_clause: u.where_clause.as_ref().map(|_| Expr::Other(String::new())),
    }
}
...
fn delete(d: &pg_query::protobuf::DeleteStmt, _text: &str) -> DeleteStmt {
    ...
    DeleteStmt {
        table,
        where_clause: d.where_clause.as_ref().map(|_| Expr::Other(String::new())),
    }
}

fn alter_table(a: &pg_query::protobuf::AlterTableStmt) -> AlterTableStmt {
    AlterTableStmt {
        table: a.relation.as_ref().map(rangevar_to_tableref).unwrap_or_default(),
    }
}
