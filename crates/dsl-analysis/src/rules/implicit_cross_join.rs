...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql014" }
    ...
    fn check(
        ...
        let names: Vec<String> = s
            ...
            .flat_map(|t| {
                ...
                if let Some(a) = &t.alias { v.push(a.clone()); }
                v
            })
            .collect();
...
fn collect_qualifiers(e: &Expr, out: &mut Vec<String>) {
    match e {
        ...
        Expr::Column { .. } => {}
        Expr::BinaryOp { left, right, .. } => { collect_qualifiers(left, out); collect_qualifiers(right, out); }
        Expr::Call { args, .. } => { for a in args { collect_qualifiers(a, out); } }
        _ => {}
    }
}
