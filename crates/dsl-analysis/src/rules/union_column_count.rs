...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql010" }
    ...
    fn check(
        ...
        let mut counts: Vec<(usize, TextRange)> = arms
            .iter()
            .map(|(s, e)| (project_count(&slice[*s..*e]), TextRange::new(
                TextSize::from(start + *s as u32),
                TextSize::from(start + *e as u32),
            )))
            .collect();
        let first = counts[0].0;
        if first == 0 { return; } // bail when we cannot determine projection count
        for (n, r) in counts.drain(1..) {
            if n != 0 && n != first {
                out.push(Diagnostic {
                    ...
                    message: format!(
                        "UNION arm projects {n} columns, first arm projected {first}; counts must match"
                    ),
                    range: r,
                });
            }
...
fn split_set_arms(text: &str) -> Vec<(usize, usize)> {
    ...
    while i < n {
        ...
        if c == '\'' { i = skip_string(bytes, i); continue; }
        if c == '-' && i + 1 < n && bytes[i + 1] == b'-' { i = skip_line_comment(bytes, i); continue; }
        if c == '/' && i + 1 < n && bytes[i + 1] == b'*' { i = skip_block_comment(bytes, i); continue; }
        if c == '(' { i = skip_parens(bytes, i); continue; }
...
fn project_count(arm: &str) -> usize {
    ...
    // Strip leading `(`
    while i < n && bytes[i] == b'(' { i += 1; i = skip_spaces(bytes, i); }
    if !word_at(bytes, i, b"SELECT") { return 0; }
    i += ident_len(bytes, i);
    ...
    while i < n {
        let c = bytes[i] as char;
        if c == '\'' { i = skip_string(bytes, i); saw_token = true; continue; }
        if c == '-' && i + 1 < n && bytes[i + 1] == b'-' { i = skip_line_comment(bytes, i); continue; }
        if c == '/' && i + 1 < n && bytes[i + 1] == b'*' { i = skip_block_comment(bytes, i); continue; }
        if c == '(' { depth += 1; i += 1; continue; }
        if c == ')' { depth -= 1; i += 1; if depth < 0 { break; } continue; }
        if depth == 0 {
            if c == ',' { commas += 1; i += 1; continue; }
            // Stop at FROM / WHERE / GROUP / ORDER / LIMIT / UNION / INTERSECT / EXCEPT
            for kw in [b"FROM" as &[u8], b"WHERE", b"GROUP", b"ORDER", b"LIMIT",
                       b"UNION", b"INTERSECT", b"EXCEPT", b"HAVING", b"WINDOW"] {
                if word_at(bytes, i, kw) {
                    return if saw_token { commas + 1 } else { 0 };
                }
            }
        }
        if !c.is_whitespace() { saw_token = true; }
        i += 1;
    }
...
fn word_at(bytes: &[u8], i: usize, kw: &[u8]) -> bool {
    ...
    if !head.eq_ignore_ascii_case(kw) { return false; }
    let after = i + kw.len();
    if after < bytes.len() {
        let c = bytes[after];
    ...
    if i > 0 {
        let p = bytes[i - 1];
        if p.is_ascii_alphanumeric() || p == b'_' { return false; }
    }
    true
}
...
fn skip_parens(bytes: &[u8], mut i: usize) -> usize {
    ...
    while i < bytes.len() {
        match bytes[i] {
            b'(' => { depth += 1; i += 1; }
            b')' => { depth -= 1; i += 1; if depth == 0 { return i; } }
