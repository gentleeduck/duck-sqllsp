...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql068" }
    ...
    fn check(
        ...
        // Find leading BEGIN.
        let mut i = 0;
        while i < n && (bytes[i] as char).is_ascii_whitespace() { i += 1; }
        if i + 5 > n || &upper[i..i + 5] != "BEGIN" { return; }
                ...
                if word == "COMMIT" || word == "END" || word == "ROLLBACK" {
                    if count == 2 {
                        ...
                        out.push(Diagnostic {
                            ...
                            message: "transaction wraps a single statement -- per-statement implicit txn already handles atomicity".into(),
                            range: text_size::TextRange::new(
                                (begin_off as u32).into(),
                                ((begin_off + 5) as u32).into(),
                            ),
                        });
                    }
                    return;
