...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql135" }
    ...
    fn check(
        ...
        if !trimmed.starts_with("SET ROLE ") && trimmed != "SET ROLE" { return; }
        // Already SET ROLE NONE / RESET? Skip.
        if trimmed.contains("NONE") { return; }
        ...
        let reset_at = after_upper.find("RESET ROLE")
            .or_else(|| after_upper.find("SET ROLE NONE"));
        let end_tx_at = after_upper.find("COMMIT")
            .or_else(|| after_upper.find("ROLLBACK"));
        let needs_warn = match (reset_at, end_tx_at) {
            (Some(_r), None) => false,
            (Some(r), Some(c)) => r > c,
            (None, _) => true,
        };
        if !needs_warn { return; }
        ...
        out.push(Diagnostic {
            ...
            message: "SET ROLE without matching RESET ROLE -- the elevated role outlives the transaction on pooled connections".into(),
