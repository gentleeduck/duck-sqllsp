...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql130" }
    ...
    fn check(
        ...
        if !trimmed.starts_with("TRUNCATE") { return; }
        // Are we inside a BEGIN..COMMIT window?
        let before_upper = source[..start].to_ascii_uppercase();
        let last_begin = before_upper.rfind("BEGIN")
            .or_else(|| before_upper.rfind("START TRANSACTION"));
        let Some(begin_at) = last_begin else { return };
        if before_upper[begin_at..].find("COMMIT").is_some()
            || before_upper[begin_at..].find("ROLLBACK").is_some() { return; }
        // Look back from begin_at..start for another TRUNCATE.
        let between = &before_upper[begin_at..start];
        if !between.contains("TRUNCATE") { return; }
