...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql062" }
    ...
    fn check(
        ...
        if !contains_word(&upper, "SAVEPOINT") { return; }
        ...
        let released = matches_release(&full_upper, &name)
            || matches_rollback_to(&full_upper, &name);
        if !released {
                    ...
                    && (i + 9 == n || !is_word(upper_bytes[i + 9] as char))
                {
                    ...
                    if body[s..j].eq_ignore_ascii_case(&name) {
                        ...
                        out.push(Diagnostic {
                            ...
                            message: format!(
                                "SAVEPOINT `{name}` is never released or rolled back to in this buffer"
                            ),
