...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql050" }
        ...
        for col in &ct.columns {
            if RESERVED.contains(&col.name.to_ascii_uppercase().as_str()) {
                ...
                out.push(Diagnostic {
                    ...
                    severity: Severity::Hint,
                    message: format!(
                        "column `{}` is a PG reserved word -- every reference will need double quotes",
                        col.name
                    ),
                    range,
                });
...
fn find_after(prefix: &str, body: &str, body_offset: usize, name: &str) -> Option<text_size::TextRange> {
    ...
    let mut e = n_start;
    while e < bytes.len() && (bytes[e].is_ascii_alphanumeric() || bytes[e] == b'_' || bytes[e] == b'.' || bytes[e] == b'"') {
        e += 1;
    }
...
fn find_column_name(body: &str, body_offset: usize, name: &str) -> Option<text_size::TextRange> {
    ...
    for (k, &b) in bytes.iter().enumerate().skip(open) {
        match b {
            b'(' => depth += 1,
            b')' => { depth -= 1; if depth == 0 { close = k; break; } }
            _ => {}
        }
    }
    ...
    while i < close {
        while i < close && (bytes[i].is_ascii_whitespace() || bytes[i] == b',') { i += 1; }
        // Skip CONSTRAINT prefix entries.
        let s = i;
        while i < close && (bytes[i].is_ascii_alphanumeric() || bytes[i] == b'_') { i += 1; }
        let word = &body[s..i];
        if word.eq_ignore_ascii_case(name) {
            return Some(text_size::TextRange::new(
                ((body_offset + s) as u32).into(),
                ((body_offset + i) as u32).into(),
            ));
        }
