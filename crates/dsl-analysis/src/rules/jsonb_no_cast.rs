...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql107" }
    ...
    fn check(
        ...
        if !after_trim.starts_with("'{") && !after_trim.starts_with("'[") { return; }
        // Walk back from `=` to find the column expression. If it
        // contains `->` or `->>`, flag.
        let before = &body[..eq_at];
        let upper_before = &upper[..eq_at];
        if !(upper_before.contains("->>") || upper_before.contains("->")) { return; }
        // Skip if user already cast (`::text =` or `::jsonb =` before
        // the equals).
        let trimmed_before = before.trim_end();
        if trimmed_before.ends_with("::text") || trimmed_before.ends_with("::jsonb") { return; }
        // Skip if RHS is `::jsonb` cast.
        if after_trim.contains("::jsonb") || after_trim.contains("::text") { return; }
