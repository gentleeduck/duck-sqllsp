...
impl LintRule for Rule {
    fn code(&self) -> &'static str { "sql056" }
        ...
        while i + 5 <= n {
            if &upper[i..i + 5] == "UNION" {
                ...
                if prev_ok && next_ok {
                    ...
                    if upper_rest.starts_with("ALL")
                        || upper_rest.starts_with("DISTINCT")
                    {
                        i = j;
                        continue;
                    }
                    ...
                    out.push(Diagnostic {
                        ...
                        message: "plain `UNION` deduplicates; use `UNION ALL` when duplicates are impossible (faster, clearer intent)".into(),
