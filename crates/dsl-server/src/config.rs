...
impl Default for Dialect {
    fn default() -> Self { Dialect::Postgresql }
}
...
impl DuckSqllspConfig {
    ...
    pub fn effective_dialect(&self) -> Dialect {
        if let Some(d) = self.dialect { return d; }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum Case { Upper, Lower, Preserve }

fn default_keyword()    -> Case { Case::Upper }
fn default_function()   -> Case { Case::Lower }
fn default_type()       -> Case { Case::Upper }
fn default_identifier() -> Case { Case::Preserve }
...
impl DuckSqllspConfig {
    ...
    pub fn merge_from(&mut self, other: DuckSqllspConfig) {
        ...
        if other.style.keyword != default.keyword     { self.style.keyword = other.style.keyword; }
